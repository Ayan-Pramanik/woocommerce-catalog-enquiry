...
class Frontend {
    ...
    public function __construct() {
        $this->settings = CE()->options_general_settings;
        $this->settings_button = CE()->options_button_appearence_settings;
        $this->settings_form = CE()->options_form_settings;
        $this->exclusion = CE()->options_exclusion_settings;
        //enqueue scripts
        ...
        $this->available_for = '';
        if (isset($this->exclusion['woocommerce_userroles_list']) && Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_userroles_list'], 'multiselect')) {
            if (is_array(Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_userroles_list'], 'multiselect'))) {
                foreach (Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_userroles_list'], 'multiselect') as $user_list_key) {
        ...
        if (isset($this->exclusion['woocommerce_user_list']) && Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_user_list'], 'multiselect')) {
            if (is_array(Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_user_list'], 'multiselect'))) {
                if (in_array($current_user->ID, Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_user_list'], 'multiselect'))) {
                    $this->available_for = $current_user->ID;
                }
            }
        }
        ...
        $for_user_type = isset($this->settings['for_user_type']) ? Utill::mvx_catalog_get_settings_value($this->settings['for_user_type'], 'select') : '';
        ...
        if (isset($this->settings['is_enable']) && Utill::mvx_catalog_get_settings_value($this->settings['is_enable'], 'checkbox') == "Enable" && ($this->available_for == '' || $this->available_for == 0)) {
    ...
    public function redirect_cart_checkout_on_conditions() {
        global $post;
        $exclusion = CE()->options_exclusion_settings;
        $current_user = wp_get_current_user();
        $user_id = $current_user->ID;
        ...
        if (isset($this->settings['is_enable']) && Utill::mvx_catalog_get_settings_value($this->settings['is_enable'], 'checkbox') == "Enable") {
            if (isset($this->settings['is_hide_cart_checkout']) && Utill::mvx_catalog_get_settings_value($this->settings['is_hide_cart_checkout'], 'checkbox') == "Enable") {

                if (isset($this->exclusion['woocommerce_userroles_list']) && Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_userroles_list'], 'multiselect')) {
                    if (is_array(Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_userroles_list'], 'multiselect'))) {
                        $count1 = count(Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_userroles_list'], 'multiselect'));
                    }
                }
                if (isset($this->exclusion['woocommerce_user_list']) && Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_user_list'], 'multiselect')) {
                    if (is_array(Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_user_list'], 'multiselect'))) {
                        $count2 = count(Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_user_list'], 'multiselect'));
                    }
                }
                ...
                if ($count2 == 0 && $count1 == 0) {
                    ...
                    if ( isset($this->exclusion['woocommerce_userroles_list']) && Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_userroles_list'], 'multiselect')  && !in_array($current_user->roles[0], Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_userroles_list'], 'multiselect') ) ) {
                        if (is_page((int) $cart_page_id) || is_page($checkout_page_id)) {
                            wp_redirect($home_url_link);
                            exit;
                        }
                    }
                    if (Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_user_list'], 'multiselect') && !in_array($current_user->ID, Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_user_list'], 'multiselect'))) {
                        if (is_page((int) $cart_page_id) || is_page($checkout_page_id)) {
                            wp_redirect($home_url_link);
                            exit;
                        }
            ...
            if (isset($this->settings_button['button_type'])) {
                switch ($this->settings_button['button_type']) {
                    case 2:
                        ...
                        if (isset($this->settings['is_enable_out_of_stock']) && Utill::mvx_catalog_get_settings_value($this->settings['is_enable_out_of_stock'], 'checkbox') == "Enable") {
    ...
    public function init_catalog() {
        ...
        if (isset($this->settings['is_enable']) && Utill::mvx_catalog_get_settings_value($this->settings['is_enable'], 'checkbox') == "Enable" && ($this->available_for == '' || $this->available_for == 0) && apply_filters( 'woocommerce_catalog_enquiry_free_active', true )) {
            add_action('init', array($this, 'remove_add_to_cart_button'));
            if (isset($this->settings['is_enable_enquiry']) && Utill::mvx_catalog_get_settings_value($this->settings['is_enable_enquiry'], 'checkbox') == "Enable") {
                $piority = apply_filters('woocommerce_catalog_enquiry_button_possition_piority', 100);
                if (isset($this->settings['is_disable_popup']) && Utill::mvx_catalog_get_settings_value($this->settings['is_disable_popup'], 'checkbox') == "Enable") {
            ...
            if (isset($this->settings['is_remove_price_free']) && Utill::mvx_catalog_get_settings_value($this->settings['is_remove_price_free'], 'checkbox') == "Enable") {
    ...
    public function change_permalink_url_for_selected_product() {
        ...
        if (isset($this->exclusion['woocommerce_product_list']) && Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_product_list'], 'multiselect')) {
            if (is_array(Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_product_list'], 'multiselect')) && isset($post->ID)) {
                if (in_array($post->ID, Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_product_list'], 'multiselect'))) {
                    $product_for = $post->ID;
                } else {
                    $product_for = '';
                }
        ...
        $category_for = '';
        if (isset($this->exclusion['woocommerce_category_list']) && Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_category_list'], 'multiselect')) {
            if (is_array(Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_category_list'], 'multiselect'))) {
                if (isset($product)) {
                    $term_list = wp_get_post_terms($post->ID, 'product_cat', array('fields' => 'ids'));

                    if (count(array_intersect($term_list, Utill::mvx_catalog_get_settings_value($this->exclusion['woocommerce_category_list'], 'multiselect'))) > 0) {
        ...
        <div id="woocommerce-catalog" name="woocommerce_catalog" >	
            ...
            <div id="responsive"  class="catalog_enquiry_form" tabindex="-1">
                <div class="modal-header">
                    <?php if (Utill::mvx_catalog_get_settings_value($this->settings_form['is_override_form_heading'], 'checkbox')) { ?>
                ...
                <div class="modal-body">  
                    ...
                    <p id="loader-after-sumitting-the-form" ><img src="<?php echo CE()->plugin_url; ?>assets/images/loader.gif" ></p>
    ...
    public function add_form_for_enquiry() {
        global $woocommerce, $post, $product, $wp_version;

        if (isset($this->settings_button)) {
            $custom_design_for_button = isset($this->settings_button['is_button']) ? Utill::mvx_catalog_get_settings_value($this->settings_button['is_button'], 'checkbox') : '';
            $enquiry_button_text = isset($this->settings_button['enquiry_button_text']) ? $this->settings_button['enquiry_button_text'] : __('Send an enquiry', 'woocommerce-catalog-enquiry');
            if ($enquiry_button_text == '') {
                $enquiry_button_text = __('Send an enquiry', 'woocommerce-catalog-enquiry');
            }
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close">&times;</button>
                                <?php if (isset($this->settings_form['is_override_form_heading']) && Utill::mvx_catalog_get_settings_value($this->settings_form['is_override_form_heading'], 'checkbox')) { ?>
                                    <?php if (isset($this->settings_form['custom_static_heading'])) { ?>
                                <h2><?php echo str_replace( "PRODUCT_NAME", $product_name, $this->settings_form['custom_static_heading'] ); ?></h2>
                                    <?php } ?>
                                <?php } else { ?>
    ...
    public function add_to_cart_button_for_selected_product() {
        ...
        if ($product_for == $post->ID || $category_for == $post->ID) {
            ...
            if (isset($this->settings_button['button_type']) && Utill::mvx_catalog_get_settings_value($this->settings_button['button_type'], 'select')) {
                add_filter('woocommerce_loop_add_to_cart_link', array($this, 'woocommerce_loop_add_to_cart_link'), 99, 3);
            } else {
                remove_action('woocommerce_after_shop_loop_item', 'woocommerce_template_loop_add_to_cart', 10);
            }
    ...
    function frontend_scripts() {
        $frontend_script_path = CE()->plugin_url . 'assets/frontend/js/';
        $frontend_script_path = str_replace(array('http:', 'https:'), '', $frontend_script_path);
        $pluginURL = str_replace(array('http:', 'https:'), '', CE()->plugin_url);
        ...
        if (isset($this->settings['is_enable']) && Utill::mvx_catalog_get_settings_value($this->settings['is_enable'], 'checkbox') == "Enable") {
            ...
            wp_enqueue_script('wce_frontend_js', $frontend_script_path . 'frontend.js', array( 'jquery', 'jquery-blockui' ), CE()->version, true);
            ...
            wp_localize_script(
                ...
                'redirect_link' => isset($this->settings['redirect_page_id']) ? get_permalink(Utill::mvx_catalog_get_settings_value($this->settings['redirect_page_id'], 'select')) : '',
                'captcha' => $captcha,
            )));
        }
    }
    ...
    function frontend_styles() {
        ...
        $frontend_style_path = CE()->plugin_url . 'assets/frontend/css/';
        $frontend_style_path = str_replace(array('http:', 'https:'), '', $frontend_style_path);
        $suffix = defined('SCRIPT_DEBUG') && SCRIPT_DEBUG ? '' : '.min';
        $custom_button_css = $button_hover_css = $button_css = '';
        // Enqueue your frontend stylesheet from here
        if (isset($this->settings['is_enable']) && Utill::mvx_catalog_get_settings_value($this->settings['is_enable'], 'checkbox') == "Enable") {
            wp_enqueue_style('wce_frontend_css', $frontend_style_path . 'frontend.css', array(), CE()->version);
