...
import Selecty, { SelectChangeEvent } from '@mui/material/Select';
...
export default class DynamicForm extends React.Component {
	...
	constructor( props ) {
		...
		this.removenestedchildOption =
			this.removenestedchildOption.bind( this );
		...
		this.OnDuplicateSelectChange =
			this.OnDuplicateSelectChange.bind( this );
		// sortable change
	...
	removeSelectboxOption( e, index, key ) {
		this.state.mvx_registration_fileds_list[ index ].options.splice(
			key,
			1
		);
		this.setState( {
	...
	handleOnChangedada( mtarget, e, target ) {
		...
		if ( Object.entries( tables_value ) ) {
			this.setState( {
				[ mtarget ]: Object.entries( tables_value ),
			} );
		}
		...
		/*this.setState({
			btn_size: target === 'btn_size' ? e.target.value : this.state.btn_size,
			font_size: target === 'font_size' ? e.target.value : this.state.font_size,
			border_radious: target === 'border_radious' ? e.target.value : this.state.border_radious,
			border_size: target === 'border_size' ? e.target.value + 'px' : this.state.border_size,
			top_gradient_color: target === 'top_gradient_color' ? e.target.value : this.state.top_gradient_color,
			bottom_gradient_color: target === 'bottom_gradient_color' ? e.target.value : this.state.bottom_gradient_color,
			border_color: target === 'border_color' ? e.target.value : this.state.border_color,
			hover_background_color: target === 'hover_background_color' ? e.target.value : this.state.hover_background_color,
			text_color: target === 'text_color' ? e.target.value : this.state.text_color,
			hover_text_color: target === 'hover_text_color' ? e.target.value : this.state.hover_text_color,
			active_background_color: target === 'active_background_color' ? e.target.value : this.state.active_background_color
		});*/
	}
	...
	onnestedChange(
		...
		if ( nestedchild === 'childnested' ) {
			if ( filedsdetails.type === 'checkbox' ) {
				itemsnested[ index ].nested_datas[ childindex ][
					filedsdetails.key
				] = e.target.checked;
	...
	componentDidMount() {
		...
		if (
			...
			catalogappLocalizer.store_location_enabled &&
	...
	renderForm = () => {
		...
		const formUI = model.map( ( m, index ) => {
						...
						<tr className="mvx-settings-tr-wrap">
							...
							<td className="mvx-settings-td-wrap">
								<div className="mvx-toggle-checkbox-content">
									...
									/>
									<label
										htmlFor={ `mvx-toggle-switch-${ o.id }` }
									></label>
								</div>
							</td>
							<td className="mvx-settings-td-wrap">
			...
			if ( type === 'custom_fileds' ) {
				input = (
					<div
						className="mvx-form-vendor-register"
						onClick={ ( e ) =>
							this.setState( {
								registration_global_select_dropdown_open: false,
							} )
					...
					>
						<p className="mvx-settings-metabox-description-code">
							Username and Password fileds display as per
													...
													<div className="mvx-registration-content">
															...
															{ registration_json_value.hidden ? (
																<div className="mvx-question-input-items">
																	<div className="dropdown">
																		<div
																			className="mvx-registration-select"
																			onClick={ (
																				e
																			) => (
																				e.stopPropagation(),
																		...
																		>
																			<span>
																				{ registration_json_value.typelabel ? (
																					<>
																						<i
																							className={ `mvx-font ${ registration_json_value.typeicon }` }
																						/>
																						{
																							registration_json_value.typelabel
																						}
																		...
																		{ this
																			.state
																			.registration_global_select_dropdown_open ? (
																			<div className="mvx-registration-dropdown-menu">
																				{ catalogappLocalizer.settings_page_string[
																					...
																					(
																						...
																						>
																							<i
																								className={ `mvx-font ${ question_content.icon }` }
																							/>
																							{
														...
														{ registration_json_value.hidden ? (
															<div className="mvx-footer-icon-form">
																<i
																	className="mvx-catalog icon-vendor-form-copy"
																	onClick={ (
																		e
																	) => {
																		this.OnDuplicateSelectChange(
																...
																2 ? (
																	<i
																		className="mvx-catalog icon-vendor-form-delete"
																		onClick={ (
																			e
																		) =>
																...
																<i
																	className="mvx-catalog icon-vendor-form-add"
																	onClick={ (
																		e
																	) =>
				...
				input = (
					<div className="mvx-form-select-field-wrapper">
						<Select
							...
							isDisabled={ m.disable ? true : false }
							onChange={ ( e ) => {
								this.onChange(
									e,
									m.key,
			...
			if ( type === 'datatable' ) {
				...
				m.column_header.map( ( data_zone, index_zone ) => {
					...
					data_zone.last_action === 'last_action_trigger'
						? ( data_zone.cell = ( row ) => (
								<div className="mvx-vendor-action-icon">
									{ data_zone.action_icons_data.map(
										( o ) => (
											<div
												onClick={ ( e ) =>
													axios( {
											...
											>
												<div data-title={ o.name }>
													<i
														className={ `mvx-font ${ o.icon }` }
													></i>{ ' ' }
												</div>
											</div>
										)
					...
					set_for_dynamic_column_zone = columns_vendor_zone_list;
					...
					<div className="mvx-datatable-button-table-wrapper">
						{ m.new_button_title ? (
							<div className="mvx-page-title">
								<a
									class="mvx-btn btn-purple"
									href={ m.new_button_link }
								>
									<i class="mvx-font icon-add"></i>
									{ m.new_button_title }
								</a>
							</div>
						) : (
						...
						<div className="mvx-backend-datatable-wrapper">
							<DataTable
								columns={ columns_vendor_zone_list }
								data={ m.column_data }
								pagination
							/>
						</div>
			...
			if ( type === 'file' ) {
				input = (
					<div className="mvx-setting-file-uploader-class">
						...
						<img
							src={
								value ? value : catalogappLocalizer.default_logo
							}
							width={ m.width }
							height={ m.height }
						/>
						...
						<div className="mvx-form-actions">
							...
							>
								...
								{ this.state.from_loading && (
									<span>
										{
											catalogappLocalizer.global_string
												.saving
										}
									</span>
								) }
